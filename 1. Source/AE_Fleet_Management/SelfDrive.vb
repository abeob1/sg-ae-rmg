                ...
                If pVal.EventType = SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED Then
                    ...
                    If pVal.ItemUID = "242" And (pVal.FormMode = SAPbouiCOM.BoFormMode.fm_OK_MODE Or pVal.FormMode = SAPbouiCOM.BoFormMode.fm_UPDATE_MODE) Then
                        ...
                        Try
                            ...
                            If oDay < oDate.Day Then
                                sMonth = oDate.Month + 1
                            Else
                                sMonth = oDate.Month ' + 1
                            End If

                            'If oDate.Day >= 20 Then
                    ...
                    If pVal.ItemUID = "1" And (pVal.FormMode = SAPbouiCOM.BoFormMode.fm_ADD_MODE Or pVal.FormMode = SAPbouiCOM.BoFormMode.fm_UPDATE_MODE) Then
                        Try
                            ...
                            If oMatrx.RowCount = 0 Then
                                ...
                                End If
                            Else
                                If opt.Selected = True And String.IsNullOrEmpty(oMatrx.Columns.Item("V_4").Cells.Item(1).Specific.String) Then
                                    oform.Items.Item("209").Specific.String = oform.Items.Item("203").Specific.String
                                End If
                            End If
                ...
                If pVal.EventType = SAPbouiCOM.BoEventTypes.et_COMBO_SELECT Then
                    ...
                    If pVal.ItemUID = "237" Then
                        Try
                            ...
                            If Not String.IsNullOrEmpty(oform.Items.Item("Item_109").Specific.String) Then
                                dMonthlyrental = oform.Items.Item("Item_109").Specific.String
                            Else
                                Oapplication_SD.SetStatusBarMessage("Rental Amount should not be Empty ....!", SAPbouiCOM.BoMessageTime.bmt_Short, True)
                                BubbleEvent = False
                                Exit Sub
                            End If

                            iNoDays = DateDiff(DateInterval.Day, oDate, oDate.AddMonths(1))  'System.DateTime.DaysInMonth(oDate.Year, oDate.Month)
                            ''  iNoDays = System.DateTime.DaysInMonth(oDate.Year, oDate.Month)
    ...
    "T0.U_AE_Bcode like '" & customer & "' and isnull(T0.U_AE_Vmodel,'') like '" & Vmodel & "' "
        ...
        If pVal.FormUID = "SDINV" Then
            ...
            ElseIf pVal.Before_Action = True Then
                ...
                If pVal.EventType = SAPbouiCOM.BoEventTypes.et_MATRIX_LINK_PRESSED And pVal.ItemUID = "Item_6" And pVal.ColUID = "V_1INV" Then

                    Dim oform As SAPbouiCOM.Form = Oapplication_SD.Forms.ActiveForm
                    Dim omatrix As SAPbouiCOM.Matrix = oform.Items.Item("Item_6").Specific
                    Dim sDocNum As String = omatrix.Columns.Item("V_1INV").Cells.Item(pVal.Row).Specific.String
                    Dim oformAR As SAPbouiCOM.Form = Nothing
                    Try
                        Oapplication_SD.ActivateMenuItem("2053")
                        oformAR = Oapplication_SD.Forms.ActiveForm
                        oformAR.Freeze(True)
                        oformAR.Mode = SAPbouiCOM.BoFormMode.fm_FIND_MODE
                        oformAR.Items.Item("8").Specific.String = sDocNum
                        oformAR.Items.Item("1").Click(SAPbouiCOM.BoCellClickType.ct_Regular)
                        oformAR.Freeze(False)
                    Catch ex As Exception
                        oformAR.Freeze(False)
                        Oapplication_SD.StatusBar.SetText(ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
                        BubbleEvent = False
                ...
                If pVal.EventType = SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED Then
                    If pVal.ItemUID = "Item_5" Then
                        ...
                        Try
                            ...
                            dDate = DateTime.ParseExact(oform.Items.Item("Item_8").Specific.value, "yyyyMMdd", Nothing)
...
"WHERE  DAY(T0.[U_AE_NXDT]) >= '" & oform.Items.Item("Item_1").Specific.String & "' and DAY(T0.[U_AE_NXDT]) <= '" & oform.Items.Item("Item_3").Specific.String & "' and T0.[U_AE_Status] = 'Open' and " & _
"T0.DocNum not in (select rtrim(substring(TT.NumAtCard , 3 , len(Tt.NumAtCard) -2)) from OINV tt where month(tt.DocDate) = '" & oform.Items.Item("12").Specific.selected.description & "' and year(tt.DocDate) = '" & dDate.Year() & "' and left(TT.NumAtCard,2) = 'SD' and isnull(TT.U_AE_BCC,'N') = 'N'  ) " & _
"group by T0.[DocNum], T0.[U_AE_Bname], " & _
"T0.[U_AE_Bcode], T0.[U_AE_Vregno], T0.[U_AE_Rno], T0.[U_AE_Contract], T0.[U_AE_NXDT], T0.[U_AE_Vdatein],T0.[U_AE_Rate], T0.[DocEntry],T0.[U_AE_Sdate] "
                            ...
                            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Recurring Billing " & Str_query, "Recurring Billing ()")
                            Try
                                oform.DataSources.DataTables.Add("@AE_SBOOKING")
                            Catch ex As Exception
                            End Try
                    ...
                    If pVal.ItemUID = "Item_7" Then
                        ...
                        Try
                            ...
                            Oapplication_SD.StatusBar.SetText("Please wait Validation in process ", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
                            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Starting Function", sFunName)
                            For mjs As Integer = 1 To oMAtrix.RowCount
                                oCheck = oMAtrix.Columns.Item("Col_5m").Cells.Item(mjs).Specific
                                If oCheck.Checked = True Then
                                    If Not String.IsNullOrEmpty(oMAtrix.Columns.Item("V_0ca").Cells.Item(mjs).Specific.String) Then
                                        dAmount = oMAtrix.Columns.Item("V_0ca").Cells.Item(mjs).Specific.String
                                        If dAmount <= 0 Then
                                            Oapplication_SD.SetStatusBarMessage("Amount should not be Zero .....! RA No. " & oMAtrix.Columns.Item("BNO").Cells.Item(mjs).Specific.String, SAPbouiCOM.BoMessageTime.bmt_Short, True)
                                            BubbleEvent = False
                                            Exit Sub
                                        End If
                                    Else
                                        Oapplication_SD.SetStatusBarMessage("Amount should not be Empty .....! RA No. " & oMAtrix.Columns.Item("BNO").Cells.Item(mjs).Specific.String, SAPbouiCOM.BoMessageTime.bmt_Short, True)
                                        BubbleEvent = False
                                        Exit Sub
                                    End If
                                End If
                            ...
                            For mjs As Integer = 1 To oMAtrix.RowCount
                                ...
                                If oCheck.Checked = True Then
                                    ...
                                    Else
                                        Oapplication_SD.SetStatusBarMessage("", SAPbouiCOM.BoMessageTime.bmt_Short, True)
    ...
    Private Function SD_InvoiceGeneration(ByVal CardCode As String, ByVal NumAtCard As String, ByVal Description As String, ByVal AcountCode As String, ByVal Taxcode As String, ByVal LineTotal As Double, _
        ...
        Try
            ...
            Dim orset As SAPbobsCOM.Recordset = Ocompany_SD.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            orset.DoQuery("SELECT T0.[U_AE_CDW], T0.[U_AE_CDW1], T0.[U_AE_PAI],T0.[U_AE_PAI1], isnull(T0.[U_AE_Ocharges],0) [U_AE_Ocharges], isnull(T0.[U_AE_Rcharg],0) [U_AE_Rcharg], isnull(T0.[U_AE_petrol],0) [U_AE_petrol] , U_AE_NBD FROM [dbo].[@AE_SBOOKING]  T0 WHERE T0.[DocEntry]  = '" & DocEntry_ & "'")
            ...
            inoofdays = orset.Fields.Item("U_AE_NBD").Value
            ...
            dEDate = dSDate.AddDays(inoofdays - 1)
            ...
            oInvoice.UserFields.Fields.Item("U_AE_BCC").Value = "Y"
    ...
    Private Function SD_InvoiceGeneration(ByVal CardCode As String, ByVal NumAtCard As String, ByVal Description As String, ByVal AcountCode As String, ByVal Taxcode As String, ByVal LineTotal As Double, _
        ...
        Try
            ...
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Posting Date " & sPDate, "")
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("ADate " & ADate, "")
            ...
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Generating the Invoice for RA ", NumAtCard)
            oInvoice.SaveXML(IO.Directory.GetCurrentDirectory & "\invoice.xml")
            val = oInvoice.Add
